...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(steps):
            ...
            if np.random.random() < self._get_epsilon(i/steps):
                A = env.action_space.sample()
            else:
                A = np.random.choice(np.ravel(
                    np.argwhere(Q[S, :] == np.amax(Q[S, :]))))
            ...
            if R_arr.size == s:
                rewards[s_counter] = np.mean(R_arr)
                R_arr = np.array([])
                s_counter += 1

            N[S, A] += 1
            Q[S, A] += 1/N[S, A] * (R + self.discount * np.amax(Q[S_new, :])
                                    - Q[S, A])
