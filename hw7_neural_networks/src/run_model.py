...
def run_model(model, running_mode='train', train_set=None, valid_set=None, test_set=None,
    ...
    """
    allowable_modes = ['train', 'test']
    if running_mode not in allowable_modes:
        raise ValueError(f"Invalid mode. Must be in {allowable_modes}.")
    ...
    if running_mode == 'train':
        ...
        while epoch < n_epochs:
            ...
            model, train_loss, train_accuracy = _train(model, train_loader,
                                                       optimizer)
            ...
            if valid_set:
                ...
                new_valid_loss, valid_accuracy = _test(model, valid_loader)

                loss['valid'].append(new_valid_loss)
                acc['valid'].append(valid_accuracy)
        ...
        loss, acc = _test(model, test_loader)
...
def _train(model, data_loader, optimizer, device=torch.device('cpu')):
    ...
    for data in data_loader:
        ...
        inputs = inputs.float()  # fixes weird type error
        ...
        loss.backward()  # backprop, gets the new gradients
