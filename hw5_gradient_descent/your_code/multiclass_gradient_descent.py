...
class MultiClassGradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        for class_ in c:
            ova_targets = np.where(targets != class_, -1, 1)
            gd = GradientDescent(self.loss, regularization=self.regularization,
                                 learning_rate=self.learning_rate,
                                 reg_param=self.reg_param)
            gd.fit(features, ova_targets)
            self.model.append(gd)
    ...
    def predict(self, features):
        ...
        Returns:
            predictions - (np.array) A 1D array of predictions of length N,
                ...
                features.
        """
        confidences = np.zeros((len(self.classes), features.shape[0]))

        for i in range(len(self.classes)):
            model = self.model[i]
            confidences[i, :] = self.model[i].confidence(features)

        c_idxs = confidences.argmax(axis=0)
        predictions = np.array([self.classes[c_idxs[i]] for i in range(len(c_idxs))])
