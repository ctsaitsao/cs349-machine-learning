...
class GradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        prev_loss = -math.inf
        curr_loss = self.loss.forward(features, self.model, targets)

        self.loss_list.append(curr_loss)
        self.accuracy_list.append(accuracy(targets, self.predict(features_static)))

        while abs(curr_loss - prev_loss) > 1e-4 and iters < max_iter:

            if batch_size:
                examples = np.concatenate((features, np.vstack(targets)), 1)
                np.random.shuffle(examples)

                features_shuffled = examples[:, :-1]
                targets_shuffled = examples[:, -1]
    ...
    def predict(self, features):
        ...
        """
        features = np.append(features, np.ones((features.shape[0], 1)), axis=1)
        predictions = np.array([1 if np.dot(self.model, feature) > 0 else -1 for feature in features])
    ...
    def confidence(self, features):
        ...
        """
        features = np.append(features, np.ones((features.shape[0], 1)), axis=1)
        confidence = np.array([np.dot(self.model, feature) for feature in features])
