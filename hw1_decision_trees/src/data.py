...
def load_data(data_path):
    ...
    features = data[:, :-1]
    targets = data[:, -1]
    attribute_names = list(np.genfromtxt(data_path, delimiter=',', dtype=str, max_rows=1))[:-1]
...
def train_test_split(features, targets, fraction):
    ...
    Returns train_features (size NxK), train_targets (1xN), test_features (size MxK 
    where M is the remaining points in data), and test_targets (1xM).
