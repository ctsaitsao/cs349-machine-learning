...
class GMM():
    def __init__(self, n_clusters, covariance_type):
        ...
        self.means = None  # keep in mind self.means is a list of np.ndarrays
    ...
    def _e_step(self, features):
        ...
        """
        all_posteriors = np.zeros((features.shape[0], self.n_clusters))

        for i in range(all_posteriors.shape[1]):
            all_posteriors[:, i] = self._posterior(features, i)
    ...
    def _m_step(self, features, assignments):
        ...
        for i in range(self.n_clusters):
            mixing_weights[i] = np.sum(assignments[:, i]) / features.shape[0]

        means = [np.zeros((features.shape[1])) for i in range(self.n_clusters)]
        for i in range(self.n_clusters):
            for j in range(features.shape[1]):
                means[i][j] = np.sum(features[:, j] * assignments[:, i]) / np.sum(assignments[:, i])
        ...
        for i in range(self.n_clusters):
            full_covariance = np.dot((features - means[i]).T * assignments[:, i], (features - means[i])) / np.sum(assignments[:, i])

            if self.covariance_type == 'spherical':
                covariances[i] = np.mean(np.diag(full_covariance))
            else:
                covariances[i] = np.diag(full_covariance)
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        return np.log(self.mixing_weights[k_idx]) \
            + multivariate_normal.logpdf(features, self.means[k_idx], self.covariances[k_idx])
    ...
    def _posterior(self, features, k):
        ...
        Returns:
            np.ndarray -- Posterior probabilities for the selected Gaussian k, of size
                (n_samples,).
        """
        num = self._log_likelihood(features, k)  # includes prior + likelihood
        denom = np.array([
